...
CASES = [
    {
        ...
        "prompt": (
            ...
            "or any other path. Run: python -m pytest -q tests/test_provider_cooldown.py. "
            ...
            "finish only after the stated tests pass."
        ),
    },
    {
        ...
        "prompt": (
            ...
            "Do not modify tests, configuration, dependencies, benchmark files, Git metadata, "
            "or any other path. Run: python -m pytest -q tests/test_business_signal_evidence.py. "
            ...
            "or search strategy. Make the smallest coherent fix and finish only after the "
            "stated tests pass."
        ),
    },
]
...
def scrubbed_kilo_env(state_dir: Path) -> dict[str, str]:
    keep = {
    ...
    # Deliberately do not pass KILO_API_KEY, GitHub tokens, or any other secret.
    return env
...
def main() -> int:
    ...
    for case in CASES:
        ...
        with tempfile.TemporaryDirectory(prefix=f"kilo-bench-{case['name']}-") as tmp:
            env = scrubbed_kilo_env(Path(tmp))
            try:
                kilo = run(
                    [
                        "kilo", "run", "--auto",
