...
def test_pexels_used_first_when_key_present(monkeypatch):
    ...
    monkeypatch.setattr(fetch_broll, "_get", lambda url, headers=None, timeout=20: json.dumps(payload).encode())
    monkeypatch.setattr(fetch_broll, "openverse", lambda q, k="": pytest.fail("openverse ne doit pas etre appele"))
...
def test_provider_failure_falls_through(monkeypatch):
    monkeypatch.setenv("PEXELS_API_KEY", "cle")
    monkeypatch.setattr(fetch_broll, "pexels", lambda q, k: (_ for _ in ()).throw(OSError("reseau")))
    monkeypatch.setattr(fetch_broll, "openverse", lambda q: {"provider": "openverse", "url": "u", "credit": "c",
                                                             "source": "s", "licence": "cc0 1.0"})
...
def test_main_writes_images_credits_and_job(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(fetch_broll, "fetch_one", lambda query: {
