...
- **Navigateur des agents** (`agents/web_guard.py`) : pages publiques dans un contexte éphémère sans cookies (headless) ; domaines de comptes (`ACCOUNT_DOMAINS`) dans le profil connecté, en lecture seule. Après la lecture d'un compte, plus aucune page hors comptes dans la même exécution ou mission. Chaque navigation, redirection HTTP comprise, est vérifiée avant la requête. Hôtes locaux et réseaux privés refusés. Sandbox Chromium réactivée (`--no-sandbox` retiré). Les résultats web sont marqués « non fiables ».
