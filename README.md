...
OmniRoute fournit un endpoint OpenAI-compatible et propose également des routes audio/vision selon les providers connectés ; le provider réellement choisi reste dépendant de la configuration OmniRoute. citeturn946008search2turn946008search1
...
RunPod Serverless utilise le modèle asynchrone `POST /run` puis `GET /status/{id}` ; le projet conserve cette identité jusque dans le manifest. citeturn623013search7
...
Ce choix suit les recommandations Playwright actuelles pour le routage réseau et l'interception fiable des contextes. citeturn623013search0turn623013search1turn623013search2
